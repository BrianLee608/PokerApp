...
public class Hand {
	...
	private void startStreet(PokerGame game, int streetIn, int startingIndex) {
		...
		if(streetIn == PRE_FLOP){
			//Set utg as last to act preflop (allows utg to initially act)
			game.players[startingIndex].setEndAction(true);
		}
		else{
			//Set SB as last to act post flop (allows SB to initially act)
			game.players[game.sbIndex].setEndAction(true);
		}
